...
int main() {
    ...
    uint32_t nnz_offset = DPU_INPUT_ARGUMENTS.nnz_offset;
    ...
    for (i=start_row; i < start_row + rows_per_tasklet; i++) {
        ...
        for (j=0; j < *current_row - prev_row; j++) {
            ...
            // Read the next non-zero element
        ...
        // When 8-byte alignement is satisfied, write the output values in MRAM
    ...
    // Store last output vector element if needed
    if (write_indx != 0) {
