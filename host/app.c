...
#endif
//#define NR_ELEM_PER_DPU (/NR_DPUS)
...
//static struct COOMatrix* TempMatrix;
static val_dt* x;
static val_dt* z;
static val_dt* y;
...
void find_partitions(uint32_t n, uint32_t *horz_partitions, uint32_t vert_partitions) {
    ...
    printf("vert:%d", dpus_per_vert_partition);
...
int main(int argc, char **argv) {
    ...
    /**************************ciphertext and verification Tags********************************/
	for(int i=0; i< (B->nnz); i++){
		//for (int j=0; j< (B->ncols);j++){
			printf("%d, %d, %d  ",B->nnzs[i].rowind,B->nnzs[i].colind,B->nnzs[i].val);
		//}
		printf("\n");
	}
    ...
    //tag
    uint8_t s[]={ 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
    s[0]=(uint8_t)(B->nnzs);
    uint8_t *firstTag = (uint8_t *) malloc((uint64_t) ((B->ncols) * sizeof(uint8_t))); 
    ...
    /*for (int j=0; j< (B->ncols); j++){
        int flag=0;
        for (int i = 0; i < (B->nnz); i++){
            if((B->nnzs[i].colind)==j){
                firstTag[j] += ((B->nnzs[i].val) * pow(s[0],(B->nnzs[i].rowind))); 
                flag=1;
            }
        }
        if(flag==0) firstTag[j]=0;
        flag=0;
        
    }*/

    for (int i = 0; i < (B->nnz); i++){
    printf("firstTag[%d]:%u\n",(B->nnzs[i].colind),firstTag[(B->nnzs[i].colind)]);
    uint8_t power = pow(s[0],(B->nnzs[i].rowind)+1);
    	uint8_t mul = (B->nnzs[i].val) * power;
        firstTag[(B->nnzs[i].colind)] = firstTag[(B->nnzs[i].colind)] + mul; 
        printf("(B->nnzs[%d].val):%d     pow: %u    mul:%u   ",i,(B->nnzs[i].val), power, mul);
        printf("firstTag[%d]:%u\n",(B->nnzs[i].colind),firstTag[(B->nnzs[i].colind)]);
    }

    int countTags=0;
    for(int i=0;i< (B->ncols); i++){
        if(firstTag[i]!=0) countTags++;
        
	}
printf("cipher done\n");
    
	
    /*************************end of ciphertext and verification Tags**************************/
    /***************************add tags to matrix*******************/
    ...
    B->nnzs = (struct elem_t *) realloc(B->nnzs, ((B->nnz)+countTags + 8)*sizeof(struct elem_t));


    for( int i=0; i< (B->ncols); i++){
        if(firstTag[i]!=0){
        B->nnzs[k].rowind = (B->nrows)-1; // Convert indexes to start at 0
        B->nnzs[k].colind = i; // Convert indexes to start at 0
        B->nnzs[k].val = (val_dt)firstTag[i]; 
        //printf("row: %d ,col: %d, val:%d", B->nnzs[k].rowind,B->nnzs[k].colind,B->nnzs[k].val);
        k++;
        }
    ...
    /***************************end of adding tags to matrix***********************************/
    sortCOOMatrix(B);
    //sortCOOMatrix(TempMatrix);
    printf("dpus:%d  ", nr_of_dpus);
    printf("vert:%d \n ", p.vert_partitions);
...
printf("col copy \n");
	...
	for(int i=0;i< max_nnz_val_per_dpu * nr_of_dpus; i++){
		...
		first[i] = (uint8_t)A->dval + (sizeof(val_dt)*i);
...
printf("copy vec done\n");
    ...
    DPU_FOREACH(dpu_set,dpu,each_dpu){
         uint32_t tile_vert_indx = i / A->horz_partitions; 
        if(each_dpu==0 || each_dpu==1 )
            DPU_ASSERT(dpu_prepare_xfer(dpu, tagCipher +  tile_vert_indx * A->tile_width));
    }
    DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, max_rows_per_dpu * sizeof(val_dt), tile_width_pad * sizeof(val_dt), DPU_XFER_DEFAULT));
*/
        //DPU_ASSERT(dpu_push_xfer(struct dpu_set_t set, dpu_xfer_t xfer, const char *symbol_name, uint32_t symbol_offset, size_t length, dpu_xfer_flags_t flags)
		//DPU_ASSERT(dpu_copy_to(dpu,"tagCipher",0, &tagCipher[each_dpu  * (ncols_pad/nr_of_dpus)],(ncols_pad/nr_of_dpus )* sizeof(uint8_t)));
	//}
    ...
    //uint8_t *hostTagResult = (uint8_t *) calloc(nrows_pad, sizeof(uint8_t));
    ...
    /*for (i = 0; i < A->nrows; i++){
        hostTagResult[i] += tagTemp[i]* x [i]; 
    }*/

    //for(int i=0;i< max_nnz_val_per_dpu * nr_of_dpus/16; i++){
        /*for(int j=0; j<ncols_pad; j++){
            hostTagResult[j]=tagTemp[j] * x[j];
...
#pragma omp parallel for num_threads(p.nthreads) shared(A, z, y, max_rows_per_dpu, c, r) private(t)
    ...
    //uint8_t *totalTag = (uint8_t *) calloc(nrows_pad, sizeof(uint8_t)); 
    //uint8_t *totalTagF = (uint8_t *) calloc(nrows_pad, sizeof(uint8_t)); 
    printf("total\n");
    for (i = 0; i < A->nrows; i++) {
...
//#if CHECK_CORR
    ...
    if(total[A->nrows]== y_host[A->nrows]) 
        printf("[" ANSI_COLOR_GREEN "Data Verified" ANSI_COLOR_RESET "] Tags are equal\n");
    else
        printf("[" ANSI_COLOR_RED "Data Not Verified" ANSI_COLOR_RESET "] Tags are not equal\n");
    ...
    printf("free temp\n");
    free(ciphertext);
    printf("free cipher text\n");
    free(hostResult);
    printf("free host result\n");
    free(total);
    printf("free total\n");
    //free(tagTemp);
    printf("free tag temp\n");
    //free(tagCipher);
    printf("free tag cipher\n");
    free(firstTag);
    printf("free first tag\n");
//#endif
